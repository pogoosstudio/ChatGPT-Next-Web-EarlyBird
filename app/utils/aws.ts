...
// Bedrock model families for validation
const BEDROCK_MODEL_FAMILIES = {
  AMAZON: "amazon",
  ANTHROPIC: "anthropic",
  META: "meta",
  MISTRAL: "mistral",
...
export function getBedrockEndpoint(
  ...
  const endpoint = `${baseEndpoint}/model/${modelId}/invoke${
    shouldStream ? "-with-response-stream" : ""
  }`;
...
export function extractMessage(res: any, modelId: string = ""): string {
  ...
  // Get model family from ID
  const family = modelId.split(".")[1]?.toLowerCase() || "";

  try {
    switch (family) {
      case "amazon":
        if (modelId.includes("nova")) {
          return res.output?.message?.content?.[0]?.text || res.output || "";
      ...
      case "anthropic":
        return res.content?.[0]?.text || res.completion || "";

      case "meta":
        return res.generation || res.text || "";

      case "mistral":
        return res.choices?.[0]?.message?.content || res.output || "";

      default:
        // Fallback to checking common response patterns
        return (
          res.output?.text ||
          res.output ||
          res.response ||
  ...
  // If no specific handler matched but we have a response
  return res.output || res.response || res.message || "";
}
